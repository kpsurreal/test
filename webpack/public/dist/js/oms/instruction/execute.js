...
var instructionExecutePage = void 0;
var tradeControllor = void 0;
var market = 'marketA';

// 指令执行表格数据
var instructionExecuteData = {
  typeStr: '',
  order: '',
  order_by: '',
  display_rules: [{
...
$(window).on('multi_load', function (event) {
  ...
  Vue.component('vue-instruction-execute', {
    ...
    methods: {
      ...
      readyShowList: function readyShowList() {
        ...
        var arr = [];
        this.ajax_list.forEach(function (e) {
          var obj = {};
  ...
  // 订单列表
  var orginfo_theme = window.LOGIN_INFO.org_info.theme; // 非3表示机构版
  Vue.component('vue-order-list', {
    ...
    computed: {
      ...
      entrustGridList: function entrustGridList() {
        var _this = this;
        var str = 'entrustGrid';
      ...
      dealGridList: function dealGridList() {
        var str = 'dealGrid';
        var rtn = this[str + '_midGridList'];
        var show_batchs = [];
      ...
      // 委托成交汇总
      entrustAndDealGridList: function entrustAndDealGridList() {
      ...
      // 委托A
      entrustListAGridList: function entrustListAGridList() {
      ...
      // 委托H
      entrustListHGridList: function entrustListHGridList() {
      ...
      // 可撤单（已废弃）
      revertGridList: function revertGridList() {
        var str = 'revertGrid';
        var rtn = this[str + '_midGridList'];
        var show_batchs = [];
    ...
    methods: {
      hideStockType: function hideStockType(val) {
        return val.split(/\./)[0];
      },
      displayStockClass: function displayStockClass(row, rule) {
      ...
      checkStockType: function checkStockType(value) {
      ...
      displayStockType: function displayStockType(value) {
        var type = this.checkStockType(value);
        if (3 == type) {
          return '沪港通';
        }
      ...
      changeShow: function changeShow(row) {
        if (row.is_batch) {
      ...
      checkDisabled: function checkDisabled(row) {
      ...
      doRevert: function doRevert() {
        ...
        this.activeGridList.forEach(function (e) {
          if (e.is_batch != true && e.web_checked && !+e.cancel_status && [1, 2, 4].some(function (el) {
            ...
            doRevertArr.push(e);
            htmlArr.push('<tr style="border-bottom: 1px solid #E2E2E2;"><td class="cell-left">' + e.product_name + '</td>' + '<td class="cell-left">' + e.stock_id + ' ' + e.stock_name + '</td>' + '<td class="cell-left">' + _this.entrustTypeCH(e.entrust_type) + '</td>' + '<td class="cell-right">' + _this.numeralNumber(e.entrust_price, 3) + '</td>' + '<td class="cell-right">' + e.entrust_amount + '</td></tr>');
          }
        });
        ...
        var confirmHtml = '<table class="custom_confirm"><tbody style="width:100%;display: inline-table;"><tr><th class="cell-left">策略</th>' + '<th class="cell-left">证券</th>' + '<th class="cell-left">买卖标志</th>' + '<th class="cell-right">委托价格</th>' + '<th class="cell-right">委托数量(股)</th></tr>' + html + '</tbody></table>';
      ...
      checkResults: function checkResults(results) {
        ...
        filter_res.fail.forEach(function (e) {
          htmlArr.push('<tr style="border-bottom: 1px solid #E2E2E2;"><td class="cell-left max-width-120" title="' + e.product_name + '">' + e.product_name + '</td>' + '<td class="cell-left">' + e.stock_id + ' ' + e.stock_name + '</td>' + '<td class="cell-left">' + _this.entrustTypeCH(e.entrust_type) + '</td>' + '<td class="cell-right">' + _this.numeralNumber(e.entrust_price, 3) + '</td>' + '<td class="cell-right">' + e.entrust_amount + '</td>' + '<td>' + e.cancel_res.msg + '</td>' + '</tr>');
        });

        var html = htmlArr.join('');
        var confirmHtml = '<table class="custom_confirm"><tbody style="width:100%;display: inline-table;">' + '<tr>' + '<th class="min-width-40 max-width-120 cell-left">策略</th>' + '<th class="min-width-40 cell-left">证券</th>' + '<th class="min-width-40 cell-left">买卖标志</th>' + '<th class="min-width-40 cell-right">委托价格</th>' + '<th class="min-width-40 cell-right">委托数量(股)</th>' + '<th class="min-width-40 cell-right">备注</th>' + '</tr>' + html + '</tbody></table>';
      ...
      chgSelect: function chgSelect(row) {
        if (row.is_batch) {
      ...
      getGridList: function getGridList(str) {
        var _this = this;
        var rtn = [];
        ...
        for (var batch_no in batch_orders) {
          ...
          }, _defineProperty(_batch, 'created_at', first_order.created_at), _defineProperty(_batch, 'updated_at', first_order.updated_at), _defineProperty(_batch, 'product', {
            name: '多策略',
            length: batch_orders[batch_no].length
          }), _defineProperty(_batch, 'all_status', []), _defineProperty(_batch, 'cancel_status', ""), _defineProperty(_batch, 'all_cancel_status', []), _defineProperty(_batch, 'can_cancel', false), _defineProperty(_batch, 'all_finish_status', []), _batch);
          ...
          [1, 2, 3, 4, 5, 6, 7, 8, 9].forEach(function (status_no) {
            batch.all_status.filter(function (status) {
              return status == status_no;
            }).length == batch.all_status.length && (batch.status = status_no);
          });
          ...
          // 这里的枚举实在是不清楚什么含义
          if (!batch.status) {
            batch.status = /0/.test(batch.all_finish_status.join('')) ? 101 : /8/.test(batch.all_status.join('')) ? 108 : 105;
          }
      ...
      // 判断是否已经结束
      isFinish: function isFinish(order) {
        if (order.status == 3) {
          return true;
        }
        if (order.status == 5) {
      ...
      entrustTypeCH: function entrustTypeCH(v) {
        switch (v.toString()) {
          case '1':
            return '买入';
          case '2':
      ...
      entrustTypeClass: function entrustTypeClass(v) {
        switch (v.toString()) {
          case '1':
            return 'red';
          case '2':
      ...
      //     return false;
      // },
      statusClass: function statusClass(v) {
      ...
      entrustModelType: function entrustModelType(v) {
        switch (v.toString()) {
          case '1':
            return '限价';
          case '2':
          ...
          case '5':
            return '市价(即时成交剩余撤单)';
          case '6':
            return '市价(最优五档即时成交剩余撤单)';
          case '7':
            return '市价(全额成交或撤单)';
          case '8':
            return '市价(最优五档即时成交剩余转限)';
        }
      },
      unixTime: function unixTime(unixTimeStamp) {
        ...
        return moment().format('YYYY-MM-DD') === momentor(unixTimeStamp).format('YYYY-MM-DD') ? momentor(unixTimeStamp).format('HH:mm:ss') : momentor(unixTimeStamp).format('M月D日 HH:mm:ss');
      },
  ...
  Vue.component('vue-trade-form', {
    ...
    mounted: function mounted() {
      ...
      $(window).on('trade-5__priceClick', function (event) {
        _this4.trade_price = event.price;
        _this4.chgTradePrice();
      });
    },
    methods: {
      setPriceAndAmount: function setPriceAndAmount(price, volume) {
        this.trade_price = price;
        this.trade_num = volume;
        this.chgTradePrice();
        this.chgTradeNum();
      },
      setCheckSubmitDisabled: function setCheckSubmitDisabled(value) {
        this.checkSubmitDisabled = value;
      },
      setTotalInfo: function setTotalInfo(obj) {
        this.showOrHide = obj.showOrHide;
        this.can_trade_num_total = obj.trade_num;
      },
      setFormData: function setFormData(obj) {
      ...
      chgTradeNum: function chgTradeNum() {
  ...
  Vue.component('vue-trade-5', {
    ...
    methods: {
      setPriceAndAmount: function setPriceAndAmount(str, index) {
      ...
      getTipStr: function getTipStr() {
        ...
        if ('single-sell' == this.cur_active) {
          ...
          _volume = Math.floor(_volume / this.ajaxStockDetail.trading_unit) * this.ajaxStockDetail.trading_unit;
      ...
      loadData: function loadData() {
        ...
        commonAjax((window.REQUEST_PREFIX || '') + "/oms/helper/stock_detail?stock_id=" + this.stock_id).then(function (_ref) {
          var code = _ref.code,
              msg = _ref.msg,
              data = _ref.data;
  ...
  Vue.component('vue-trade-revert', {
    ...
    methods: {
      ...
      doRevert: function doRevert() {
        ...
        this.activeGridList.forEach(function (e) {
          if (e.is_batch != true && e.web_checked && !+e.cancel_status && [1, 2, 4].some(function (el) {
            ...
            htmlArr.push('<tr style="border-bottom: 1px solid #E2E2E2;"><td class="cell-left">' + e.product_name + '</td>' + '<td class="cell-left">' + e.stock_id + ' ' + e.stock_name + '</td>' + '<td class="cell-left">' + e.bs_symbol_text + '</td>' + '<td class="cell-left">' + e.quote_type_text + '</td>' + '<td class="cell-right">' + e.entrust_price + '</td>' + '<td class="cell-right">' + e.entrust_amount + '</td></tr>');
          }
        });
        ...
        var html = htmlArr.join('');
        var confirmHtml = '<table class="custom_confirm"><tbody style="width:100%;display: inline-table;"><tr><th class="cell-left">策略</th>' + '<th class="cell-left">证券</th>' + '<th class="cell-left">买卖标志</th>' + '<th class="cell-left">报价方式</th>' + '<th class="cell-right">委托价格</th>' + '<th class="cell-right">委托数量(股)</th></tr>' + html + '</tbody></table>';
      ...
      checkResults: function checkResults(results) {
        ...
        var htmlArr = [];
        filter_res.fail.forEach(function (e) {
          htmlArr.push('<tr style="border-bottom: 1px solid #E2E2E2;"><td class="cell-left max-width-120" title="' + e.product_name + '">' + e.product_name + '</td>' + '<td class="cell-left">' + e.stock_id + ' ' + e.stock_name + '</td>' + '<td class="cell-left">' + _this.entrustTypeCH(e.entrust_type) + '</td>' + '<td class="cell-right">' + e.entrust_price + '</td>' + '<td class="cell-right">' + e.entrust_amount + '</td>' + '<td>' + e.cancel_res.msg + '</td>' + '</tr>');
        });
      ...
      getGridList: function getGridList(str) {
        ...
        for (var batch_no in batch_orders) {
          ...
          var batch = (_batch2 = {
            ...
            // 报价方式
            entrust_model: first_order.entrust_model
          }, _defineProperty(_batch2, 'created_at', first_order.created_at), _defineProperty(_batch2, 'updated_at', first_order.updated_at), _defineProperty(_batch2, 'product', {
            name: '多策略',
            length: batch_orders[batch_no].length
          }), _defineProperty(_batch2, 'all_status', []), _defineProperty(_batch2, 'cancel_status', ""), _defineProperty(_batch2, 'all_cancel_status', []), _defineProperty(_batch2, 'can_cancel', false), _defineProperty(_batch2, 'all_finish_status', []), _batch2);
    ...
    methods: {
      ...
      loadData: function loadData() {
        ...
        var loadStockIndexs = function loadStockIndexs() {
          commonAjax(url).then(function (_ref2) {
            var code = _ref2.code,
                msg = _ref2.msg,
                data = _ref2.data;
  ...
  Vue.component('vue-trade-preview', {
    ...
    methods: {
      checkSubmitDisabled: function checkSubmitDisabled() {
        ...
        if (this.display_arr.every(function (e) {
          return true != e.web_checked;
        })) {
          flag = true;
        }
      ...
      chgAll: function chgAll() {
        var value = this.gridHead_web_checked;
        this.display_arr.forEach(function (e) {
          e.web_checked = value;
        });
      ...
      multiProductsSubmit: function multiProductsSubmit() {
        ...
        this.display_arr.forEach(function (e) {
          if (e.web_checked) {
            htmlArr.push('<tr style="border-bottom: 1px solid #E2E2E2;"><td class="cell-left">' + e.product_name + '</td>' + '<td class="cell-left">' + typeStr1 + '</td>' + '<td class="cell-right">' + typeStr2 + '</td>' +
            ...
            '<td class="cell-right">' + e.trade_num + '</td>' + '<td class="cell-right">' + Number((e.trade_num * ins_price).toFixed(2)) + '</td></tr>');
        ...
        '<th class="cell-right">委托价格</th>' + '<th class="cell-right">交易数量(股)</th>' + '<th class="cell-right">交易金额(元)</th></tr>' + html + '</tbody></table>' + '<div class="custom_total"><span style="color:#999999;font-size:13px;">总计</span><span>' + totalAmount + '</span><span>' + totalMoney + '</span></div>';
        ...
        function checkResults(results) {
          ...
          for (var i = 0; i < results.length; i++) {
            ...
            //服务端返回数据对应理解
            var res = row.add_hand_order_res;
            //模拟数据
            // res ={"code":0,
            //         "msg":"",
            //         "data":{
            ...
            if (res.code == 0) {
              ...
              if (res.data[_this.stock_detail.stock_id]) {
                var temp = res.data[_this.stock_detail.stock_id];
          ...
          var contentChild = Vue.extend({
            data: function data() {
              ...
              var temp = Object.assign({}, results);
              //清空数量
              this.$nextTick(function () {
      ...
      refreshDisplay: function refreshDisplay() {
        ...
        if ('' == this.trade_num || '' == this.trade_price && 2 != this.trade_mode) {
          ...
          this.$nextTick(function () {
            var flag = _this5.checkSubmitDisabled();
      ...
      setSubmitData: function setSubmitData(obj) {
        ...
        if (this.select_ins.id == obj.select_ins.id) {
          ...
          var tmpSelectArr = [];
          this.select_ins.sub_list.forEach(function (e) {
            if (e.web_checked) {
              tmpSelectArr.push(e.product_id);
            }
          });
          this.select_ins = obj.select_ins;
          this.select_ins && this.select_ins.sub_list && this.select_ins.sub_list.forEach(function (e) {
            if (tmpSelectArr.some(function (el) {
              return el == e.product_id;
            })) {
              e.web_checked = true;
            }
          });
      ...
      setDealPrice: function setDealPrice(val) {
        //  设置交易价格
  ...
  var tradeFlow = new Vue({
    ...
    methods: {
      ...
      // 修改选中的指令
      chgSelectIns: function chgSelectIns(val) {
        var _this6 = this;
      ...
      chgTradeNum: function chgTradeNum(val) {
        this.trade_num = val;
        this.setSubmitData();
      },
      // End: 外部调用，获取提交时需要的数据
      // Begin: 内部处理
      // 检查数据是否ok，ok则全部交给
      setSubmitData: function setSubmitData() {
        // 将数据提交给预览模块
  ...
  // 风控持仓
  var loadRiskPositionFn = function loadRiskPositionFn(resolve) {
  ...
  // 持仓数据
  var loadPositionRealtimeFn = function loadPositionRealtimeFn(resolve) {
