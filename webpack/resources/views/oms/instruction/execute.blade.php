...
@section('content')
  <div class="main-container fc custom-main-container">
    <div>
      <section class="section-container custom-main-container__section-container" id="instruction-execute" v-cloak>
        <vue-instruction-execute ref="instruction-execute" :ajax_list="ajaxExecuteList"></vue-instruction-execute>
        <vue-order-list ref="order-list"></vue-order-list>
        ...
        <section class="create-order board black-turnoff fc fixed" style="bottom: 0px;margin-bottom:0;z-index: 50;" v-bind:style="'width:' + wholeWidth + 'px'">
          <div class="body">
            ...
            <div class="right-board" style="padding-bottom: 0px;padding: 0;min-width: 1400px;">
              <div v-show="true == showBoard && ('single-buy' == curActive || 'single-sell' == curActive)" style="display: flex;">
                <div v-show="true == showBoard && ('single-buy' == curActive || 'single-sell' == curActive)">
                  <vue-trade-form ref="trade-form" :cur_active="curActive" v-on:stock_id="chgStockId($event)"></vue-trade-form>
                </div>
                <div v-show="true == showBoard && ('single-buy' == curActive || 'single-sell' == curActive)">
                  <vue-trade-5 :stock_id="stock_id" :cur_active="curActive"></vue-trade-5>
                </div>
                <div v-show="true == showBoard && ('single-buy' == curActive || 'single-sell' == curActive)" class="vue-preview-flex">
                  <vue-trade-preview :cur_active="curActive" ref="trade-preview"></vue-trade-preview>
                </div>
              </div>
              <div v-show="true == showBoard && 'do_revert' == curActive">
                <vue-trade-revert ref="trade-revert"></vue-trade-revert>
              </div>
            </div>
          </div>
...
@section('js-private')
  ...
  <script type="text/javascript" src="{{ asset('/dist/js/oms/instruction/execute.js') }}"></script>
@endsection
