...
单组合多股票 买入卖出 交易后估算模块
trigger:
'order_create:multi_order:data_change:'+direction //foot修改计算后，再将数据返回给table，让其进行显示

on:
order_create:nav:change //下单菜单切换后，重置数据
'order_create:nav:multi-stocks:'+direction //下单菜单切换到多股票页面，使能该模块
spa_product_change
multi_products:head:updated:checked_one ／/得到单产品的数据
'create_order:multi_stocks:'+direction+':max_cash:changed'
'order_create:multi_order:real_cost_change:'+direction
...
<div class="foot" color="{{$to_buy ? 'red' : 'blue'}}">
    <div class="foot-inner">
        ...
        <div class="item submit">
            @include('oms.multi.product.order_create.multi_order.submit',['to_buy'=>$to_buy,'to_sell'=>$to_sell])
        </div>
    </div>
    <script>
    (function(){
        ...
        $(window).on('order_create:nav:change',function(){
            active = false;
            window.PRODUCT && window.PRODUCT.id && reset();
        }).on('order_create:nav:multi-stocks:'+direction,function(){
            active = true;
        }).on('spa_product_change',
            reset
        ).on('multi_products:head:updated:checked_one',function(event){
            ...
            // if( $.dirtyCheck(core_info_cache_key,core_info) ){
                ...
                data.balance_amount = product.runtime ? product.runtime.enable_cash : 0;
                updateData();
            // }
