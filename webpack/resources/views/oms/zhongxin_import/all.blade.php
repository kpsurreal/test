...
@section('asset-css')
<style media="screen">
    .zhongxin_import_table td,th{text-align:center;position:relative;height:40px;}
    .zhongxin_import_table td:first-child{width:3em;}
    .zhongxin_import_table td:nth-of-type(2){width:6em;font-size:12px;}
    .zhongxin_import_table td:nth-of-type(14){width:6em;font-size:12px;}
    select{width:100px;}
...
<table id="zhongxin_import_table" class="zhongxin_import_table" border="1" width="100%" cellpadding="1" cellspacing="1">
    <tr>
        <th></th>
        <th>成交日期</th>
        <th>运行中的策略</th>
    ...
    <tr productid="{{ $i['product_id'] }}">
        ...
        <td class="setted"><a href="{{ config('view.path_prefix','') }}/oms/zhongxin_import/{{$type}}/cancel?id={{$i['id']}}">取消</a></td>
        @else
        <td><a href="{{ config('view.path_prefix','') }}/oms/zhongxin_import/{{$type}}/set_disabled?id={{$i['id']}}">设置无效</a></td>
        @endif
...
<div style="display:none;">
    <form style="display:none;" action="/oms/zhongxin_import/{{$type}}/cancel" method="get">
        <input class="btn btn-lg btn-default" id="cancel_import_btn" type="submit" value="取消本次导入的订单">
    </form>
    <form class="form-inline" action="/oms/zhongxin_import/{{$type}}/do_import" method="post" onsubmit="return do_submit(this);">
...
$(function(){
    ...
    $('#zhongxin_import_table select').each(function(){
        ...
        //脏值检测
        $('#zhongxin_import_table select:not([disabled])').each(function(){
            $(this).val() !== $(this).attr('originval') ? $(this).closest('td').addClass('dirty') : $(this).closest('td').removeClass('dirty');
        });
        $('#zhongxin_import_table tr').removeClass('dirty').find('td.dirty').closest('tr').addClass('dirty');
    ...
    ['委托编号','成交编号'].forEach(function(field_name){
        $('#zhongxin_import_table td[key='+field_name+']').hover(function(){
        ...
        $('#zhongxin_import_table').on('click','td[key='+field_name+']',function(){
    ...
    function getStatistics(){
        var table = $('#zhongxin_import_table');
    ...
    function resetRowsBgColor(){
        $('#zhongxin_import_table').find('tr[productid][productid!=""]').each(function(){
