...
@section('js-private')
    ...
    <script src="{{ asset('/dist/js/product/base_setting.js') }}" type="text/javascript"></script>
    ...
    <script>
        ...
        FD.volume = '{{$volume}}';
        FD.base_name = '{{$base_name}}';
        FD.operator_uid = {{$operator_uid}};
        FD.auto_verify = {{$auto_verify}};
        FD.begin_capital = {{$begin_capital}};  //初始资金
        FD.collect_capital = {{$collect_capital}};
        FD.available_amount = Number({{$available_amount}}) + Number(FD.collect_capital); //可用资金
        FD.is_running = {{$is_running ? 1 : 0}};
        FD.is_forever = {{$is_forever}};
        FD.fee_mode = {{$fee_mode}};
        FD.lever_ratio = {{$lever_ratio}};
        FD.org_info = JSON.parse( $('[org-info-data]').html() );
        FD.permission = JSON.parse( $('[permission-info-data]').html() );
        $(function(){
            $.ajax({
                url: window.REQUEST_PREFIX + '/html/product/base_setting.html',
                type: 'get',
                success: function(data){
                    $('.section-container').html(data);
                    JS_base_setting(window.REQUEST_PREFIX);
                },
                error: function(){
