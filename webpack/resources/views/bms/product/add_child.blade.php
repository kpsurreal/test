...
@section('js-private')
    ...
    <script src="{{ asset('/dist/js/product/add_child.js') }}" type="text/javascript"></script>
    <script type="text/json" base-info-data >{!! isset($base_info) ? json_encode($base_info) : '' !!}</script>
    <script type="text/json" org-info-data >{!! isset($org_info) ? json_encode($org_info) : '' !!}</script>
    <script>
        var FD = {};
        FD.init_capital = {{$init_capital}};
        FD.base_info = JSON.parse( $('[base-info-data]').html() );
        FD.org_info = JSON.parse( $('[org-info-data]').html() );
        $(function(){
            $.ajax({
                url: window.REQUEST_PREFIX + '/html/product/add_child.html',
                type: 'get',
                success: function(data){
                    $('.section-container').html(data);
                    JS_add_child(window.REQUEST_PREFIX);
                },
                error: function(){
