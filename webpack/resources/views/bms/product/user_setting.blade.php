...
@section('asset-css')
    <link href="{{ asset('/js/plugin/selectize/css/selectize.default.css') }}" rel="stylesheet">
...
@section('js-private')
    ...
    <script src="{{ asset('/dist/js/product/user_setting.js') }}" type="text/javascript"></script>
    <script type="text/json" users-info-data >{!! isset($lists) ? json_encode($lists) : '' !!}</script>
    <script type="text/json" permissions-info-data >{!! isset($role_permissions) ? json_encode($role_permissions) : '' !!}</script>
    ...
    <script>
        ...
        FD.usersInfo = JSON.parse( $('[users-info-data]').html() );
        FD.rolePermissions = JSON.parse( $('[permissions-info-data]').html() );
        FD.org_info = JSON.parse( $('[org-info-data]').html() );
        FD.permission = JSON.parse( $('[permission-info-data]').html() );
        $(function(){
            $.ajax({
                url: window.REQUEST_PREFIX + '/html/product/user_setting.html',
                type: 'get',
                success: function(data){
                    $('.section-container').html(data);
                    JS_user_setting(window.REQUEST_PREFIX);
                },
                error: function(){
